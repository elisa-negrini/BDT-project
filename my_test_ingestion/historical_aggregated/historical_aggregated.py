...
from pyflink.common.runtime_mode import RuntimeExecutionMode # Importa RuntimeExecutionMode
from datetime import datetime, timedelta, time as dtime
from zoneinfo import ZoneInfo
import json
import numpy as np
...
# --- Global Data Stores (Shared across process_element calls, but ensure Flink's design handles state correctly) ---
# IMPORTANT: For a production Flink job, these global variables might not be ideal for large-scale state management
# within a KeyedProcessFunction in a parallel environment. Flink's managed state (ValueState, ListState) is preferred
# if macro_data_by_series and fundamentals_by_symbol_year grow very large or need to be fault-tolerant.
# However, for smaller pre-loaded datasets or if the data fits in memory of each parallel instance, this can work.
# Given parallelism is set to 1, these act as global stores across the entire job.
macro_data_by_series = {}
fundamentals_by_symbol_year = {}
pending_stock_batches = [] # This buffer is also global to the single parallel instance
...
class FullDayAggregator(KeyedProcessFunction):
        ...
        try:
            ...
            if "series" in message and "value" in message and "date" in message:
                try:
                    ...
                    required_macro_series = ["gdp_real", "cpi", "ffr", "t10y", "t2y", "spread_10y_2y", "unemployment"]
                    if all(s in macro_data_by_series and macro_data_by_series[s] for s in required_macro_series):
                        if not ready_flags["macro_ready"]: # Log only once
                            ready_flags["macro_ready"] = True
                            print("[INFO] Macro data ready.")
                except Exception as e:
                    print(f"[ERROR] Error processing macro data: {e} - Message: {message}", file=sys.stderr)
                return # Important: Return after processing macro data to avoid processing as stock/fundamental
            ...
            if "symbol" in message and "calendarYear" in message:
                try:
                    ...
                    if not ready_flags["fundamentals_ready"]: # Log only once
                        ready_flags["fundamentals_ready"] = True
                        print("[INFO] Fundamental data ready.")
                except Exception as e:
                    print(f"[ERROR] Error processing fundamental data: {e} - Message: {message}", file=sys.stderr)
                return # Important: Return after processing fundamental data

            # --- Process Buffered Stock Batches if Ready ---
            if ready_flags["macro_ready"] and ready_flags["fundamentals_ready"] and pending_stock_batches:
                print(f"[INFO] Processing {len(pending_stock_batches)} buffered stock batches.", file=sys.stdout)
                # It's important to iterate over a copy if you clear the original list.
                # In Flink's managed state, you'd use iterator.remove().
                # Given global lists and parallelism 1, this is functional.
            ...
            if not (ready_flags["macro_ready"] and ready_flags["fundamentals_ready"]):
                if "ticker" in message and "data" in message:
                    pending_stock_batches.append(value)
                return # Don't process live stock data if prerequisites aren't met yet
    ...
    def _process_stock_message(self, message):
        ...
        for i in range(len(parsed)):
            ...
            def mean(values): return float(np.mean(values)) if values else None
            def std(values):
                if values and mean(values) is not None and mean(values) != 0:
                    return float(np.std(values)) / mean(values)
                return None
            def total(values): return float(np.sum(values)) if values else 0.0

            macro_values = get_macro_values_for_date(now.date())
            fundamentals = get_fundamentals(row["symbol"], now.year - 1) # Assuming previous year for fundamentals
            ...
            result = {
                ...
                "price_mean_1min": mean(window_vals("open", 1)),
                "price_mean_5min": mean(window_vals("open", 5)),
                "price_std_5min": std(window_vals("open", 5)), # Modified to use helper
                "price_mean_30min": mean(window_vals("open", 30)),
                "price_std_30min": std(window_vals("open", 30)), # Modified to use helper
                "size_tot_1min": total(window_vals("volume", 1)),
                "size_tot_5min": total(window_vals("volume", 5)),
                "size_tot_30min": total(window_vals("volume", 30)),
                "sentiment_bluesky_mean_2hours": 0.0, # These remain placeholders
                "sentiment_bluesky_mean_1day": 0.0,
                "sentiment_news_mean_1day": 0.0,
                "sentiment_news_mean_3days": 0.0,
                "sentiment_general_bluesky_mean_2hours": 0.0,
...
def main():
    ...
    env = StreamExecutionEnvironment.get_execution_environment()
    env.set_parallelism(1) # Keep parallelism at 1 if using global dictionaries for macro/fundamentals
    # ESSENZIALE: Configura il job per essere eseguito in modalità batch.
    # Questo dice a Flink che la sorgente ha una fine.
    env.set_runtime_mode(RuntimeExecutionMode.BATCH)

    # --- Kafka Source Setup (modern API) ---
    kafka_source = KafkaSource.builder() \
        .set_bootstrap_servers(KAFKA_BOOTSTRAP_SERVERS) \
        .set_topics(*KAFKA_TOPICS) \
        .set_group_id("flink_batch_group") \
        .set_starting_offsets(OffsetsInitializer.earliest()) \
        .set_value_deserializer(SimpleStringSchema()) \
        .set_bounded(OffsetsInitializer.latest()) \
        .build()
    ...
    # --- Execute the Flink Job ---
    print("[INFO] Starting Flink job: Historical Aggregation to PostgreSQL with Macro Data")
    # Questo metodo si bloccherà e il job terminerà quando tutte le sorgenti bounded avranno esaurito i dati.
    env.execute("Historical Aggregation to PostgreSQL with Macro Data")
    print("[INFO] Flink job finished successfully.") # This will print after job completion
