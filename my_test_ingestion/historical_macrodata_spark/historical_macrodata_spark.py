...
# def save_to_s3(alias, df_spark):
#     output_path = f"s3a://{S3_BUCKET}/{alias}"

#     try:
#         # Estrae anno e mese solo per partizionare (le colonne saranno usate per il path ma non salvate nei file)
#         df_partitioned = df_spark.withColumn("year", year("Date")) \
#                                  .withColumn("month", month("Date")) \
#                                  .repartition("year", "month", "Date")  # un file per giorno

#         df_partitioned.write.partitionBy("year", "month") \
#                             .mode("overwrite") \
#                             .parquet(output_path)

#         print(f"✅ Salvato su MinIO: {output_path}")
#     except Exception as e:
#         print(f"❌ Errore salvataggio {alias}: {e}")
...
#     output_path = f"s3a://{S3_BUCKET}/{alias}"
#     try:
#         df_spark.write.partitionBy("year").mode("overwrite").parquet(output_path)
#         print(f"✅ Salvato su MinIO: {output_path}")
#     except Exception as e:
#         print(f"❌ Errore salvataggio {alias}: {e}")
