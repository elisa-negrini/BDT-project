...
# # Now loaded dynamically from DB, with a fallback
...
def load_tickers_from_db():
    ...
    try:
        ...
        return tickers_data # <--- This return was missing in your original snippet
...
def create_consumers():
    ...
    consumer_anomalies = KafkaConsumer(
        "anomaly_detection", # Topic name for anomalies
        bootstrap_servers="kafka:9092",
        value_deserializer=lambda x: json.loads(x.decode("utf-8")),
        auto_offset_reset="latest",
        group_id="dashboard-consumer-anomalies"
    )
    return consumer_prices, consumer_predictions, consumer_anomalies # Return the new consumer too

consumer_prices, consumer_predictions, consumer_anomalies = create_consumers() # Unpack the new consumer
...
for tp, messages in records_anomalies.items():
    for msg in messages:
        data = msg.value
        # Ensure essential keys are present in the anomaly message
        ...
        try:
            ...
            # Get min_timestamp and max_timestamp as they were reported (can be out of order)
            ...
            # Ensure min_timestamp_ny is always chronologically before max_timestamp_ny for display
            # We sort them here for consistent display, even if the anomaly detection
            # reported them in a different "min/max value" order.
            ...
            st.session_state.anomaly_buffer[current_message_ticker].append({
                "timestamp": ts_utc, # This is the detection timestamp
                "anomaly_type": data.get("anomaly_type"),
                "min_price": min_price,
                "max_price": max_price,
                "price_difference": price_difference,
                "min_timestamp": chron_min_timestamp_utc, # Chronologically ordered for consistency
                "max_timestamp": chron_max_timestamp_utc # Chronologically ordered for consistency
            })
        except Exception as e:
...
# # NEW: Cleanup anomaly buffer
# for ticker in st.session_state.anomaly_buffer:
#     st.session_state.anomaly_buffer[ticker] = [
#         # Clean up based on the chronological max_timestamp, not the detection timestamp
#         d for d in st.session_state.anomaly_buffer[ticker] if d["max_timestamp"] >= cleanup_cutoff_utc
#     ]
...
# # NEW: Filter anomaly data for display and convert timestamps
# filtered_anomalies = []
# for d in st.session_state.anomaly_buffer.get(selected_ticker, []):
#     # Important: Filter based on the chronological max_timestamp of the anomaly period
#     if d["max_timestamp"] >= display_cutoff_utc:
#         d["timestamp_ny"] = d["timestamp"].astimezone(ny_timezone) # Detection timestamp
#         d["min_timestamp_ny"] = d["min_timestamp"].astimezone(ny_timezone) # Chronological start
#         d["max_timestamp_ny"] = d["max_timestamp"].astimezone(ny_timezone) # Chronological end
#         filtered_anomalies.append(d)

# --- Data Cleanup and Filtering for Display (Harmonized to 5 Minutes) ---
# Questo cutoff verrà usato sia per pulire i buffer di sessione
# che per filtrare i dati da inviare al grafico, garantendo coerenza.
display_and_cleanup_cutoff_utc = datetime.now(utc_timezone) - timedelta(minutes=5)

# Pulizia e Filtro per Prezzi
filtered_prices = []
...
# Pulizia e Filtro per Previsioni
filtered_predictions = []
...
# Pulizia e Filtro per Anomalie
filtered_anomalies = []
# Manteniamo l'anomalia nel buffer se una qualsiasi delle sue "estremità" cronologiche
# rientra nella finestra di visualizzazione.
...
if not filtered_prices:
    ...
    if filtered_predictions:
        ...
        # Add empty trace to maintain legend entry
        fig.add_trace(go.Scatter(
            x=[],
            y=[],
            mode="markers+lines",
    ...
    # NEW: Add Anomaly Shaded Regions to Chart with separate legend entries
    # Add dummy traces for legend entries for positive and negative anomaly periods
    fig.add_trace(go.Scatter(
        x=[None], y=[None], mode='lines',
        line=dict(color='rgba(0, 128, 0, 0.2)', width=0), # Invisible line matching green fill
        fill='tozeroy', # This is just to make it appear in the legend
        name='Positive Anomaly Period',
        showlegend=True
    ))
    fig.add_trace(go.Scatter(
        x=[None], y=[None], mode='lines',
        line=dict(color='rgba(255, 0, 0, 0.2)', width=0), # Invisible line matching red fill
        fill='tozeroy', # This is just to make it appear in the legend
    ...
    for anomaly in filtered_anomalies:
        # Determine color based on price_difference (value-based)
        # Positive price_difference means price went up -> Green
        # Negative price_difference means price went down -> Red
        ...
        shapes_to_add.append(
            go.layout.Shape(
                type="rect",
                xref="x", yref="paper",
                # Use the chronologically sorted timestamps for the shape's extent
                x0=anomaly["min_timestamp_ny"],
                x1=anomaly["max_timestamp_ny"],
                y0=0, # Bottom of the plot
                y1=1, # Top of the plot
                fillcolor=fill_color, # Dynamic color
                layer="below",
...
# --- Add Daily Reference Price Line to Chart as a Shape ---
    ...
    if daily_reference_price_value is not None:
        ...
        shapes_to_add.append(
            go.layout.Shape(
                ...
                line=dict(
                    color=line_color,
                    dash=line_dash,
                    width=2, # A bit thicker for better visibility
                ),
                layer="above" # THIS IS THE KEY CHANGE: Ensures it's drawn above other shapes
            )
        )
        ...
        fig.add_annotation(
            ...
            xanchor="left",
            yanchor="bottom",
            font=dict(color=line_color, size=12) # Adjusted font size for clarity
        )
    ...
    fig.update_layout(
        ...
        template="plotly_white",
        hovermode="x unified",
        shapes=shapes_to_add # All shapes (anomalies and reference line) are now in this list
    )
    ...
    fig.update_layout(
        ...
        template="plotly_white",
        hovermode="x unified",
        shapes=shapes_to_add # Add all anomaly shapes at once
    )
...
if filtered_anomalies:
    ...
    # Note: min_timestamp is now *guaranteed* to be the chronological start due to sorting earlier
    ...
    if price_diff_value < 0:
        ...
        # If you explicitly want a '+' sign for positive values:
        # price_diff_display = f"+${price_diff_value:.2f}"
        pass # By default, positive numbers don't show a '+' sign

    # Use the chronologically ordered timestamps for display
    start_time_str = last_anomaly['min_timestamp_ny'].strftime('%Y-%m-%d %H:%M:%S')
    end_time_str = last_anomaly['max_timestamp_ny'].strftime('%H:%M:%S') # Only time for end
