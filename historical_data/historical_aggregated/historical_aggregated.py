...
# --- Tracking Variables for Completion Detection ---
processed_tickers = set()
expected_days_per_ticker = None
ticker_day_counts = {}
...
class FullDayAggregator(KeyedProcessFunction):
    ...
    def _process_stock_message(self, message):
        ...
        global processed_tickers, expected_days_per_ticker, ticker_day_counts, total_processed_records

        ticker = message.get("ticker")
        num_records = len(batch_results)
        total_processed_records += num_records

        # Track days per ticker
        if ticker not in ticker_day_counts:
            ticker_day_counts[ticker] = num_records
            processed_tickers.add(ticker)
            
            # Set expected days from first ticker if not set
            if expected_days_per_ticker is None:
                expected_days_per_ticker = num_records
                print(f"[INFO] Set expected days per ticker: {expected_days_per_ticker}")
            
            print(f"[INFO] Completed processing ticker {ticker} ({len(processed_tickers)}/{len(TICKERS)} tickers)")
            
            # Check if all tickers are processed
            if len(processed_tickers) == len(TICKERS):
                print(f"[INFO] All {len(TICKERS)} tickers processed. Sending completion signal...")
    ...
    def _send_completion_signal(self):
        """Send signal to start_model topic when all data is processed"""
        try:
            ...
            producer.send('start_model', value=message)
            producer.flush()
            producer.close()
            
            print(f"[INFO] Sent completion signal to start_model topic. Total records processed: {total_processed_records}")
            
        except Exception as e:
            print(f"[ERROR] Failed to send completion signal: {e}", file=sys.stderr)
