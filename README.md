...
Questo progetto analizza in tempo reale l’andamento del mercato azionario combinando dati di borsa, indicatori macroeconomici, analisi del sentiment da news e social media. L'obiettivo è fornire una **previsione della quotazione di ogni azienda a un minuto di distanza**, visualizzabile su una dashboard interattiva.
...
Dal menu a tendina in alto puoi selezionare un'azienda e visualizzare sia l’andamento storico della quotazione che la previsione futura generata dal modello.
...
docker volume rm my_test_ingestion_pgdata
...
docker exec -it app python company_fundamentals.py
...
docker-compose -f <nome_file_docker_compose>.yml down

Ad esempio, per spegnere la configurazione stream:

docker-compose -f docker-compose-stream.yml down
...
From the dropdown menu at the top, you can select a company and view both the historical price trend and the one-minute-ahead prediction generated by the model.

## 2. Docker Compose for Historical Data and Training (docker-compose-historical.yml)

This configuration allows you to download approximately 13 million rows of historical data (~4GB) and train the model from scratch. This is a longer process but gives you full control over the model.
...
docker-compose -f <docker_compose_filename>.yml down
